...
// ================================================================
// 23. 절전 모드
// 조건 1 — 탭 숨김 / 화면 이탈 (Page Visibility API): 즉시 진입
// 조건 2 — 30초 이상 아무 입력 없음: 진입
// 복귀 시 — 절전 해제 + 자동 저장 + 탐험/전투 루프 재개
// ================================================================

(function() {
  const IDLE_MS = 30 * 1000; // 30초

  let _sleepActive  = false;
  let _idleTimer    = null;
  let _clockTimer   = null;
  let _enterTime    = null;  // 절전 진입 시각 (복귀 시 경과 시간 표시용)
  let _overlay      = null;

  // ── 절전 오버레이 DOM 생성 (최초 1회) ──
  function getOverlay() {
    if (_overlay) return _overlay;

    _overlay = document.createElement('div');
    _overlay.id = 'sleep-overlay';
    ...
    _overlay.innerHTML = `
      <div id="sleep-logo" style="
        font-size: 64px;
        filter: drop-shadow(0 0 18px rgba(93,202,165,0.4));
        animation: sleepFloat 3s ease-in-out infinite;
      ">🐉</div>
      <div id="sleep-clock" style="
        font-size: 42px; font-weight: 300; letter-spacing: 6px;
        color: #e0e0e0; font-family: 'Segoe UI', sans-serif;
      ">00:00</div>
      <div id="sleep-date" style="
        font-size: 13px; color: #4a6090; letter-spacing: 2px;
        font-family: 'Segoe UI', sans-serif;
      "></div>
      <div style="
        font-size: 11px; color: #2a3a5a; margin-top: 8px;
        letter-spacing: 1.5px; font-family: 'Segoe UI', sans-serif;
      ">CAU 푸앙 월드 · 절전 중</div>
      <div id="sleep-away-time" style="
        font-size: 11px; color: #3a5a3a; margin-top: 2px;
    ...
    // 클릭 또는 키 입력으로 복귀
    _overlay.addEventListener('click', wakeUp);
  ...
  function updateClock() {
    ...
    const awayEl = document.getElementById('sleep-away-time');
    if (!clock) return;
    ...
    const date  = now.getDate();
    if (dateEl) dateEl.textContent = months + '월 ' + date + '일 ' + days[now.getDay()];

    if (_enterTime && awayEl) {
      const elapsed = Math.floor((now - _enterTime) / 1000);
      const em = String(Math.floor(elapsed / 60)).padStart(2, '0');
      const es = String(elapsed % 60).padStart(2, '0');
      awayEl.textContent = '자리 비운 시간: ' + em + ':' + es;
    }
  }
  ...
  function sleep() {
    ...
    // 전투 중이면 battleBusy 상태 보존 (루프는 그냥 둠 — canvas는 freeze 안 해도 됨)
    // 탐험 rAF 루프는 explore-container 숨김 체크로 자연히 멈춤
  ...
  function wakeUp() {
    ...
    // 복귀 토스트
    if (_enterTime) {
      const sec = Math.floor((new Date() - _enterTime) / 1000);
      const label = sec < 60
        ? sec + '초'
        : Math.floor(sec / 60) + '분 ' + (sec % 60) + '초';
      if (typeof showToast === 'function') {
        showToast('👋 돌아오셨네요! (' + label + ' 자리 비움)', 'info', 2500);
      }
    }

    // 탐험 루프 재개 (explore-container가 보이면)
    const exploreCont = document.getElementById('explore-container');
    if (exploreCont && exploreCont.style.display !== 'none') {
      if (typeof requestAnimationFrame === 'function' && typeof update === 'function') {
  ...
  function resetIdleTimer() {
    clearTimeout(_idleTimer);
    if (_sleepActive) return;
    _idleTimer = setTimeout(sleep, IDLE_MS);
  }

  // ── 입력 이벤트 감지 ──
  const INPUT_EVENTS = ['mousemove','mousedown','keydown','touchstart','scroll','click'];
  INPUT_EVENTS.forEach(evt => {
    document.addEventListener(evt, () => {
      if (_sleepActive) { wakeUp(); return; }
      resetIdleTimer();
    }, { passive: true });
  });

  // ── Page Visibility API — 탭 숨김/화면 이탈 즉시 절전 ──
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      sleep();
    } else {
      // 탭으로 돌아왔을 때
      if (_sleepActive) wakeUp();
    }
  });

  // ── 전역 노출 (수동 제어용) ──
  window.forceSleep = sleep;
  window.forceWakeUp = wakeUp;

  // ── DOMContentLoaded 후 유휴 타이머 시작 ──
  document.addEventListener('DOMContentLoaded', () => {
    resetIdleTimer();
  });
